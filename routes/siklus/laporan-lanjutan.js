    ...
    for (const s of siklusList) {
        ...
        if (matchedMenuId) {
          ...
          for (const br of bahanRows) {
            ...
            for (const b of activeJenjang) {
              ...
              if (!bahanMap[keyNama]) bahanMap[keyNama] = { nama: namaDisplay, nama_display: namaDisplay, per_jenjang: {}, buffer_persen: Number(br.buffer_persen) || 0, satuan: br.satuan || '', keterangan: br.keterangan || '', berat_per_satuan: Number(br.berat_per_satuan) || 0 };
      ...
      for (const g of gridDay) {
        ...
        for (const b of activeJenjang) {
          ...
          if (!bahanMap[keyNama]) bahanMap[keyNama] = { nama: namaDisplay, nama_display: namaDisplay, per_jenjang: {}, buffer_persen: Number(g.buffer_persen) || 0, satuan: g.satuan || '', keterangan: '', berat_per_satuan: Number(g.berat_per_satuan) || 0 };
          if (!bahanMap[keyNama].per_jenjang[b]) {
            bahanMap[keyNama].per_jenjang[b] = { kebutuhan_kg: 0, jumlah_siswa: jmlPm, berat_bersih: Math.round(beratPerSiswa * 100) / 100, persen_bdd: persenBdd, berat_kotor: Math.round(beratKotorPerSiswa * 100) / 100 };
          }
          bahanMap[keyNama].per_jenjang[b].kebutuhan_kg += kebutuhanKg;
