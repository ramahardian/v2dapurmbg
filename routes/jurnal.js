...
// ---- 7. HPP (Harga Pokok Produksi) per Menu ----
const roleHpp = requireRole('admin', 'keuangan', 'ahli_gizi');
router.get('/laporan/hpp', roleHpp, async (req, res) => {
  const t = req.user.tenant_id;

  // Ambil semua menu + hitung HPP dari bahan bakunya
  const [rows] = await db.query(`
    SELECT
      m.id AS menu_id,
      m.nama AS menu_nama,
      ...
      COALESCE(SUM(mb.jumlah * b.harga_satuan), 0) AS total_hpp,
      0 AS hpp_per_porsi
    FROM menu m
  ...
  const stats = {
    total_menu: rows.length,
    total_bahan: rows.reduce((s, r) => s + Number(r.jumlah_bahan), 0),
    rata_hpp: rows.length ? Math.round(rows.reduce((s, r) => s + Number(r.total_hpp), 0) / rows.length) : 0,
    total_hpp_all: rows.reduce((s, r) => s + Number(r.total_hpp), 0),
    menu_tanpa_bahan: rows.filter(r => Number(r.jumlah_bahan) === 0).length,
  };

  // Rincian bahan per menu (untuk detail)
  const detailPromises = rows.slice(0, 50).map(async (menu) => {
    if (Number(menu.jumlah_bahan) === 0) return null;
    const [bahan] = await db.query(`
      SELECT b.nama AS bahan_nama, b.satuan, b.harga_satuan,
             mb.jumlah, ROUND(mb.jumlah * b.harga_satuan, 2) AS subtotal
      FROM menu_bahan mb
      JOIN bahan_baku b ON b.id = mb.bahan_baku_id
      WHERE mb.menu_id = ?
      ORDER BY subtotal DESC
    `, [menu.menu_id]);
    return { menu_id: menu.menu_id, bahan };
  });

  const detailBahan = (await Promise.all(detailPromises)).filter(Boolean);

  res.json({
    rows,
    stats,
    detail_bahan: Object.fromEntries(detailBahan.map(d => [d.menu_id, d.bahan])),
