...
// ═══ Animasi kartu global ═══
// Setiap konten baru di #content (hasil render halaman, fetch template, dsb)
// otomatis diberi animasi masuk berstagger + count-up pada angka .dash-count.
...
function animatePageCards() {
  ...
  if (!root) return;
  const pending = [];
  // 1. Kartu yang sudah memakai class animasi eksplisit → beri stagger
  root.querySelectorAll('.dash-card-item, .dash-card-enter').forEach(el => {
    if (!el.style.animationDelay) pending.push(el);
  });
  // 2. Auto-animasi kartu konten utama (bg-white rounded-2xl) & kartu statistik gradient.
  //    Hanya yang belum beranimasi dan bukan bagian dalam kartu lain yang sudah beranimasi.
  root.querySelectorAll('.bg-white.rounded-2xl, .bg-gradient-to-br.rounded-2xl').forEach(el => {
    if (el.classList.contains('dash-card-item') || el.classList.contains('dash-card-enter')) return;
    if (el.closest('.dash-card-item, .dash-card-enter')) return;
    const isStat = el.classList.contains('bg-gradient-to-br');
    el.classList.add(isStat ? 'dash-card-item' : 'dash-card-enter');
    pending.push(el);
  });
  pending.forEach((el, i) => { el.style.animationDelay = (i * 60) + 'ms'; });
  if (typeof animateDashboardCounts === 'function') animateDashboardCounts(root);
}
...
function initContentObserver() {
  ...
  contentObserver = new MutationObserver((muts) => {
    ...
    for (const m of muts) {
      if (m.type === 'childList' && m.addedNodes.length) { shouldAnimate = true; break; }
    }
    if (shouldAnimate) {
      // Tunggu satu frame agar semua node tertanam sempurna sebelum animasi
      requestAnimationFrame(() => animatePageCards());
    }
  });
