      ...
      <div class="bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-sm mb-5">
        <div class="overflow-x-auto"><table class="w-full" style="min-width:${Math.max(600, formData.items.length * 130 + 100)}px"><thead><tr>
          <th class="w-[100px] min-w-[100px] px-3 py-3 text-left text-xs font-semibold text-stone-400 bg-stone-50 border-b border-r border-stone-200">Kelompok</th>${formData.items.map(it => {
            ...
            return '<th class="px-2 py-2.5 text-center bg-stone-50 border-b border-r border-stone-200 align-top"><div class="text-xs font-bold text-stone-700">' + it.hari_nama + '</div><div class="inline-block my-1 px-2 py-0.5 rounded-full bg-amber-100 text-[10px] font-semibold text-amber-700">Menu ' + it.hari_ke + '</div>' + (dt ? '<div class="text-[10px] text-stone-400">' + fmtDate(dt) + '</div>' : '') + (_mn ? '<div class="text-[9px] text-stone-500 mt-0.5 truncate max-w-[120px] mx-auto" title="' + escHtml(_mn) + '">' + escHtml(_mn) + '</div>' : '') + '<div id="sk-foto-' + it.hari_ke + '" class="mt-1.5">' + renderSiklusFoto(it.hari_ke, _ft) + '</div><div class="mt-2 flex flex-col gap-1"><button onclick="openMenuPickerHari(' + it.hari_ke + ')" class="w-full inline-flex items-center justify-center gap-1 px-2 py-1.5 text-[10px] font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors" title="Pilih dari Menu & Gizi">📋 Pilih Menu</button><button onclick="bukaTemplatePickerHari(' + it.hari_ke + ')" class="w-full inline-flex items-center justify-center gap-1 px-2 py-1.5 text-[10px] font-medium text-amber-700 bg-amber-50 border border-amber-200 rounded-lg hover:bg-amber-100 transition-colors" title="Pakai template menu manual yang sudah disimpan">🗂 Template</button></div></th>';
          }).join('')}
        </tr></thead><tbody>
          ${ROW_KEYS.map(rk => {
