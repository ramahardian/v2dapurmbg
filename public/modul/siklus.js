  ...
  wrap.innerHTML = selectAllHtml + list.map(s => {
    ...
    return `<div class="bg-white border border-stone-200 rounded-xl p-5 hover:shadow-lg hover:border-stone-300 transition-all duration-200 group">
      ...
      <div class="flex items-center gap-4 text-xs text-stone-500 mb-3">
        ...
        <span class="flex items-center gap-1"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M12 11v2m0 0v2m0-2h2m-2 0h-2"/></svg>${fmtPorsi(s)}</span>
      </div>
...
function fmtPorsi(s) {
  var dbPorsi = Number(s.jumlah_porsi) || 0;
  var pmPorsi = Number(s.pm_porsi) || 0;
  if (dbPorsi > 0) return fmtNum(dbPorsi) + ' porsi';
  if (pmPorsi > 0) return fmtNum(pmPorsi) + ' porsi (dari PM)';
  return '0 porsi';
}
