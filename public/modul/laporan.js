        ...
        if (items.length > 0) {
          ...
          if (menuNama) {
            menuChips = menuNama.split(' + ').filter(Boolean).map(function(x) {
              return '<span class="text-emerald-800 font-semibold">' + escHtml(x) + '</span>';
            }).join('<span class="text-emerald-400 px-0.5 shrink-0">•</span>');
          }
          ...
          var reportHeader = '<div class="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">' +
            ...
            '<div class="px-5 py-3 bg-white border-b border-stone-100">' +
              '<div class="flex flex-wrap items-center justify-center gap-2">' +
                (menuChips ? '<div class="flex flex-wrap items-center justify-center gap-x-1 gap-y-1 self-center max-w-full"><span class="text-[10px] font-bold uppercase tracking-wider text-stone-400 shrink-0 mr-1.5">MENU</span>' + menuChips + '</div>' : '') +
