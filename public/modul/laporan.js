...
const tabColors = {
  ...
  try {
    if (tab === 'persediaan') {
      ...
      try {
        var rhTanggal = (siklusInfo && siklusInfo.tanggal_mulai) ? siklusInfo.tanggal_mulai : nowDate.toISOString().slice(0, 10);
        ...
        if (items.length > 0) {
          ...
          var reportHeader = '<div class="mt-6 bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">' +
