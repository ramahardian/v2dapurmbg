...
async function runMigration() {
  ...
  } catch (e) { log('  (skip migrasi kolom)'); }
  ...
  } catch (e) { log('  (skip migrasi standar_sp)'); }
  ...
  // Copy ke tenant lain
  try {
    const [missingTenants] = await conn.query("SELECT t.id FROM tenants t LEFT JOIN sp_referensi_bahan s ON s.tenant_id = t.id WHERE s.id IS NULL");
    if (missingTenants.length) {
      ...
      for (const t of missingTenants) {
        for (const row of seedRows) {
          await conn.query("INSERT IGNORE INTO sp_referensi_bahan (tenant_id, nama, kategori, berat_bersih, bdd_persen, berat_kotor) VALUES (?,?,?,?,?,?)", [t.id, row.nama, row.kategori, row.berat_bersih, row.bdd_persen, row.berat_kotor]);
        }
    ...
    if (tenants.length && !existingAkun[0].cnt) {
      ...
      for (const t of tenants) {
        for (const [kode, nama, bp, tipe] of seedAkun) {
          await conn.query('INSERT IGNORE INTO akun (tenant_id, kode, nama, bp, tipe) VALUES (?,?,?,?,?)', [t.id, kode, nama, bp, tipe]);
        }
      }
  ...
  try {
    ...
    const [fkExists] = await conn.query("SELECT CONSTRAINT_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'karyawan_id' AND REFERENCED_TABLE_NAME IS NOT NULL");
    if (!fkExists.length) {
  ...
  if (!tExist.length) {
    ...
    log(`✓ Admin seeded: ${process.env.ADMIN_EMAIL} / ${process.env.ADMIN_PASSWORD}`);
  } else {
  ...
  try {
    const [fkExists] = await conn.query("SELECT CONSTRAINT_NAME FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'siklus_menu_item' AND CONSTRAINT_TYPE = 'FOREIGN KEY' AND CONSTRAINT_NAME = 'fk_smi_menu'");
    if (!fkExists.length) {
  ...
  // Tambah kolom kelompok & saldo_normal di akun (untuk laporan keuangan)
  try {
    const [kelCol] = await conn.query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'akun' AND COLUMN_NAME = 'kelompok'");
    if (!kelCol.length) {
      await conn.query("ALTER TABLE akun ADD COLUMN kelompok VARCHAR(50) DEFAULT 'AKTIVA' AFTER kode, ADD COLUMN saldo_normal ENUM('Debit','Kredit') DEFAULT 'Debit' AFTER kelompok");
      log('✓ Migrasi akun: tambah kolom kelompok & saldo_normal');
      // Set default values for existing akun
      await conn.query("UPDATE akun SET kelompok='AKTIVA', saldo_normal='Debit' WHERE kode LIKE '1%' AND (kelompok IS NULL OR kelompok='')");
      await conn.query("UPDATE akun SET kelompok='KEWAJIBAN', saldo_normal='Kredit' WHERE kode LIKE '2%' AND (kelompok IS NULL OR kelompok='')");
      await conn.query("UPDATE akun SET kelompok='EKUITAS', saldo_normal='Kredit' WHERE kode LIKE '3%' AND (kelompok IS NULL OR kelompok='')");
      await conn.query("UPDATE akun SET kelompok='PENDAPATAN', saldo_normal='Kredit' WHERE kode LIKE '4%' AND (kelompok IS NULL OR kelompok='')");
      await conn.query("UPDATE akun SET kelompok='BIAYA', saldo_normal='Debit' WHERE kode LIKE '5%' AND (kelompok IS NULL OR kelompok='')");
    }
  } catch (e) { log('  (skip migrasi kelompok akun)'); }

  log('✓ Schema berhasil dibuat');
  await conn.end();
  return logs;
}
