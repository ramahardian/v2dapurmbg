...
// ====================================================================
// Load data: dari URL (--url), file (--file), atau hardcoded default
// ====================================================================
// Usage:
//   node scripts/seed_sp_referensi.js
//   node scripts/seed_sp_referensi.js --url=https://example.com/sp-referensi.json
//   node scripts/seed_sp_referensi.js --file=./data.xlsx
// ====================================================================
...
const URL_SOURCE = urlFlag ? urlFlag.split('=').slice(1).join('=') : null;
const FILE_SOURCE = fileFlag ? fileFlag.split('=').slice(1).join('=') : null;

/**
 * Normalize a row from any source (JSON/CSV/XLSX) to the internal format
 */
function normalizeRow(raw) {
  // Map common column name variants
  const col = (variants, fallback = null) => {
    const v = Array.isArray(variants) ? variants : [variants];
    for (const key of v) {
  ...
  const nama = col(['nama_bahan', 'nama', 'bahan_pangan', 'bahan']);
  if (!nama) return null;

  return {
    nama_bahan: typeof nama === 'string' ? nama.trim() : String(nama),
    berat_bersih: col(['berat_bersih', 'bb', 'berat_bersih_(g)', 'berat_bersih_(gram)'], 0),
    persen_bdd: col(['persen_bdd', 'bdd', 'bdd_persen', 'bdd_persen_(%)', 'bdd_(%)'], 100),
    energi: col(['energi', 'kalori', 'energy'], null),
    protein: col(['protein'], null),
    lemak: col(['lemak', 'fat', 'lemak_(g)'], null),
    karbohidrat: col(['karbohidrat', 'karbo', 'carb', 'karbohidrat_(g)'], null),
    serat: col(['serat', 'fiber', 'serat_(g)'], null),
  };
}

/**
 * Default hardcoded data (fallback jika tidak ada --url atau --file)
 */
function getDefaultData() {
  return [
    ...
    { "nama_bahan": "Putren", "berat_bersih": 25.0, "persen_bdd": 100.0, "energi": 36.75, "protein": 1.275, "lemak": 0.175, "karbohidrat": 7.875, "serat": 0.325 },
...
/**
 * Parse CSV text ke array of objects (parser sederhana)
 * Catatan: parser ini basic — tidak handle quoted field dengan koma di dalamnya
 */
...
async function loadData() {
  ...
  console.log(`  ✓ ${data.length} baris dari data default`);
  return data;
}
