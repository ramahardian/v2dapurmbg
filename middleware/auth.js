...
async function requireAuth(req, res, next) {
  ...
  try {
    ...
    const user = rows[0];
    req.user = user;
    console.log('Auth middleware - User:', req.user.nama, 'Role:', req.user.role, 'Tenant:', req.user.tenant_id);
    next();
  } catch (e) {
...
function requireRole(...roles) {
  return (req, res, next) => {
    console.log('RequireRole middleware - User role:', req.user?.role, 'Allowed roles:', roles);
